...
1. **Replace `dokument` inside the file** — it appears in the resource
   `id` (`mii-param-dokument-manifest`). A `{{…}}` left in a FHIR `id` is
   ...
   `input/translations/de/ImplementationGuide-mii-ig-dokument.po`, which
