...
| `pagecontent/` | The IG's narrative pages as Markdown (`index.md`, `guidance.md`, …). Every page listed under `pages:` in `sushi-config.yaml` needs a file here. **English is the default content language** (EN-first, MII meta wiki Release 2027); the German mirror lives in `translations/de/pagecontent/`. | IG Publisher |
...
| `translations/` | Translation supplements. This module is **English-default with a German translation** under `translations/de/` — see `translations/README.md`. | IG Publisher (i18n) |
