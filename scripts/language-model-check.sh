...
# `input/translations/de/`, matching the other MII KDS modules —
# kerndatensatz-basis, kerndatensatz-meta, kerndatensatzmodul-biobank and
# kerndatensatzmodul-medikation all ship exactly this configuration. Prose that
# calls German the default/leading/authoritative/source language — or names
# `input/translations/en/` — contradicts the shipped configuration; it fails
# here instead of shipping to readers.
#
# NOTE: this reverses the module's earlier German-first setting, which
# implemented the MII Release-2027 "IG Umbau – DE First" decision and
# KDS-Governance v4 §4.4. The owner directed the change on 2026-09-12 to align
# with the four sibling modules. If that decision is reasserted, flip this file
# and sushi-config.yaml back together.
#
...
PATTERNS=(
  ...
  'i18n-default-lang:[^A-Za-z]{0,6}de([^A-Za-z]|$)'
...
ALLOW='scripts/language-model-check.sh|IG Umbau
scripts/language-model-check.sh|i18n-default-lang
scripts/language-model-check.sh|input/translations/en'
...
There is no input/translations/en/. Prose written while the module was
German-first is the usual source of a hit — rewrite it, never the facts.
