...
This guide is **German-language** (default language); **English** is the translation. German is therefore both the base rendering of the guide and the `/de/` rendering; use the language switcher at the top right to change between `/de/` and `/en/`.

Translated pages live under `input/translations/en/pagecontent/` (same file name as the German page); resource translations are `.po` files under `input/translations/en/`. Details: [`docs/recipes/add-translation.md`](https://github.com/medizininformatik-initiative/kerndatensatz-dokument/blob/main/docs/recipes/add-translation.md) in this repository.
